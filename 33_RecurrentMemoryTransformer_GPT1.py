    ...
    with tqdm_notebook(total=len(dataloader), desc=f"Train {epoch+1}") as pbar:
        for batch_idx, samples in enumerate(dataloader):
            ...
            with torch.set_grad_enabled(True):
                ...
                logits1, mem1, _ = model(Input_concat)        # (1, 1024, 20000), (1, 128, 512), None
                logits2, mem2, _ = model(Input_concat, mem1)  # (1, 1024, 20000), (1, 128, 512), None
                logits_lm, mem3, _ = model(Input_concat, mem2)  # (1, 1024, 20000), (1, 128, 512), None
...
def evaluate(text):
    ...
    for i in range(DECODER_LEN):
        ...
        logits1, mem1, _     = trained_model(concate_input)        # (1, 1024, 20000), (1, 128, 512), None
        logits2, mem2, _     = trained_model(concate_input, mem1)  # (1, 1024, 20000), (1, 128, 512), None
        predictions, mem3, _ = trained_model(concate_input, mem2)  # (1, 1024, 20000), (1, 128, 512), None
